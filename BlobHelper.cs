//************************************************************************************************************
//*
//* The source code in this file comes directly from the Azure Storage Samples Codeplex project
//* at http://azurestoragesamples.codeplex.com/releases/view/61325. The only changes that have 
//* been made is renaming the namespace to give it context within this solution.
//*
//************************************************************************************************************
...
namespace AzureBlobStorage
{
    public class BlobHelper : RESTHelper
    {
        ...
        public BlobHelper(string storageAccount, string storageKey) : base("https://" + storageAccount + ".blob.core.windows.net/", storageAccount, storageKey)
        {
        }
        ...
        public KeyValuePair<byte[], string> GetBlobResponse(string container, string blob)
        {
            string contentType = string.Empty;
            KeyValuePair<byte[], string> data = new KeyValuePair<byte[], string>();

            return Retry<KeyValuePair<byte[], string>>(delegate ()
            {
                HttpWebResponse response;
                ...
                try
                {
                    ...
                    content = ReadFully(response.GetResponseStream());

                    response.Close();
                    data = new KeyValuePair<byte[], string>(content,contentType);                    
